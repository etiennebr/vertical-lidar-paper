...
Light detection and ranging (LiDAR) provides information on the vertical structure of forest stands allowing for detailed, yet large-scale, ecosystem study.
One common limitation of LiDAR to interpret the link between forest structure and attributes is the high dimension of the data.
As a result, dimension reduction techniques are often used, limiting the interpretation of the results.
We quantified the influence of three forest stand attributes: species, crown cover, and age on the vertical distribution of airborne LiDAR returns of forested stands. 
We studied 5428 regular, even-aged stands in Quebec (Canada) with five dominant species: balsam fir (*Abies balsamea* (L.) Mill.), paper birch (*Betula papyrifera* Marsh), black spruce (*Picea mariana* (Mill.) BSP), white spruce (*Picea glauca* Moench) and aspen (*Populus tremuloides* Michx.). 
We modeled vertical distribution of LiDAR 
returns against the three attributes using a functional general linear model (GLM) and a novel nonparametric graphical test of significance.
...
@racine_etal14; @karna_etal19]. Airborne LiDAR is an important tool to quantify biomass and
...
[@vierling_etal08; @vierling_etal10; @seavy_etal09; @karna_etal20].
...
nadir-facing ADS–40 sensor images, the expert photo-interpreters classified
many species by integrating information on landscape positions, crown
shapes, textures, and colors. Every photo was segmented into homogenous
...
ecological knowledge of the photo-interpreters. We used this photo-interpreted forest map 
...
of the stand cover. 
Using a low threshold for the dominance increased the number of observations for the analysis which in turn increased the ability to detect effects, but also increased noise from other co-dominant species. 
...
the vertical distribution of LiDAR returns for every stand [@muss_etal11].
Vertical distribution is a histogram of LiDAR returns height sliced in 39
...
by the total number of points in the stand, so each stand vertical distribution
...
significance [@myllymaki_mrkvicka19; @mrkvicka_etal19]. 
...
\begin{equation}
d_i(h) = \beta_0(h) +X_{sp}\cdot\beta_{sp}(h) +X_{cc}\cdot\beta_{cc}(h) +X_{age}\cdot\beta_{age}(h) +\varepsilon(h)   (\#eq:full)
\end{equation}

where, for every scaled height $h$, $d_i(h)$ is the $N\times1$ vector of observed LiDAR density distributions at scaled height $h$, and
$\beta_{sp}(h)$, $\beta_{cc}(h)$, and $\beta_{age}(h)$ are the parameter vectors related to species $X_{sp}$, crown cover $X_{cc}$, and age $X_{age}$,  and $\varepsilon(h)$ is the vector of random errors with mean zero and finite variance $\sigma^2(h)$. The crown cover $X_{cc}$ was regarded as a continuous variable, while species $X_{sp}$ and age $X_{age}$---given that the last class was open---were regarded as categorical variables. Consequently, $\beta_{sp}(h)$ is the $5 \times 1$ parameter vector related to the $N \times 5$ matrix $X_{sp}$ identifying the species, $\beta_{cc}(h)$ is the parameter related to the crown cover ($X_{cc}$ is a $N \times 1$ matrix) and $\beta_{age}(h)$ is the $6 \times 1$ parameter vector related to the $6 \times 1$ matrix $X_{age}$ of the age group information.
...
-@freedman_lane83) using the methodology illustrated in Figure \@ref(fig:fig-fglm-process). We tested the effect of each variable, i.e. considered the three null hypotheses $\beta_{sp, m}(h)=0$ for all $m = 1, ..., 5$ and $h$, $\beta_{cc}(h)=0$ for all $h$, and $\beta_{age, l}(h)=0$
for all $l = 1, ..., 6$ and $h$ for the three variables, respectively. 
The coefficients of the discrete factors were constrained to sum to zero.
...
\begin{equation}
d_i(h) = \beta_0(h) +X_{cc}\cdot\beta_{cc}(h) +X_{age}\cdot\beta_{age}(h) +\varepsilon(h)    (\#eq:species)
\end{equation}

\begin{equation}
d_i(h) = \beta_0(h) +X_{sp}\cdot\beta_{sp}(h) +X_{age}\cdot\beta_{age}(h) +\varepsilon(h)   (\#eq:cc)
\end{equation}

\begin{equation}
d_i(h) = \beta_0(h) +X_{sp}\cdot\beta_{sp}(h) +X_{cc}\cdot\beta_{cc}(h) +\varepsilon(h)     (\#eq:age)
\end{equation}
...
[@myllymaki_etal17]. 
The Freedman-Lane algorithm includes all steps from the simulation under the null hypothesis to the estimation of the chosen test statistic (Figure 3).
Within the algorithm we used 2999 random permutations. The global extreme rank length envelope test (Myllymäki et al., 2017, Mrkvička et al., 2020) was then constructed from the test statistics calculated from the empirical vertical distribution of LiDAR returns and the 2999 permuted data sets.

The null hypothesis was rejected if the empirical test vector leaved the envelope at any point. To account for the tests of all three variables, we 
...
heterogeneity of group variance [@winkler_etal14], and the model residuals
indicated heterogeneity of variance for all variables.
Following @mrkvicka_etal20, we transformed the matrix of vertical LiDAR returns 
...
transformed into $S_{i,j}$ (scaled) function,

\begin{equation}
S_{i,j}(h) = \frac{d_{i,j}(h) - \bar{d_j}(h)}{\sqrt{Var(d_j(h))}} \cdot
...
Figure \@ref(fig:fig-fglm-process) summarises the steps used to isolate the effect of each variable on the vertical distribution of LiDAR returns.
...
envelope (grey bands--the test is only performed once) that shows the area of acceptance 
...
ground, and *superior* for the inverse. 
Figure \@ref(fig:fig-cc2) displays the vertical distribution of LiDAR returns of each species per crown cover (including the effect of age). 
The increase in crown cover concentrated the
...
70 yr). The older stands of 90 and 120 years of age also
had an abundance of returns in the superior part of the stand (90 yr: 38–68%; 120 yr: 33–44%). 
Stands of 90 and 120 years' vertical distribution were reduced below 28 and 23% of stand height, respectively. 
...
Figure \@ref(fig:fig-age2) displays the median vertical distribution of LiDAR returns on each species, by age class (including the crown cover effect). 
The young white and black spruces stand displayed
...
excluding the species variable (Equation \@ref(eq:species)) displayed a difference of
more than 0.10 with the full model R² between 72--92% stand height. Excluding
the crown cover variable from the model (Equation \@ref(eq:cc)) created a
difference of more than 0.10 with the full model R² between 0--21% and 46--67%
of stand height. The model excluding the age variable (Equation \@ref(eq:age))
displayed a difference of more than 0.10 with the full model R² at 18--38% and
62--64% of stand height.

```{r include = FALSE}
fig_rsq_caption <- 
"Comparison of $R²(h)$ of the full model (Equation \\@ref(eq:full)) containing all 
variables, and the three reduced models (Equations \\@ref(eq:species)--\\@ref(eq:age))
...
The effect of species associations could be controlled by using a higher dominance threshold (we used 50%), but using a 60% dominance made little differences on the conclusions, and higher threshold removed too much data for some species.
...
While fGLM is not suited to predictions, non parametric methods, such as Deep Neural Network could be good candidates for species classification because they can handle the functional representation of the data and their non-linear relations. 
...
distribution of LiDAR returns which moved returns below 33% to above 38% of stand
...
rapid transformation in the 10 year group, which gradually decelerated to
...
contribution to R² happened between 18--38% and
62--64% of stand height. Unlike @aber79 who observed 
...
variations. 
The distribution of LiDAR returns in the superior and inferior portions of the stand can be highly dependent on the LiDAR survey parameters and species crown shape [@roussel_etal17; @roussel_etal18], which might make them less likely to be stable across different surveys. 
Nonetheless, the full model (Equation \@ref(eq:full)) still explained 20% of the variability up to 90% of stand height. 
The difference in variability explained by the species model compared to
...
Making functions comparable through registration is a common problem in functional data analysis [@ramsay_etal09].
We used the highest LiDAR return of the stand to scale the vertical distribution of LiDAR returns. 
This could make the registration sensitive to extreme measures and might explain, in part, why the superior part of the canopy's R² was lower. 
The probability of hitting the tree-top varies across crown shapes, which is a an interesting property to distinguish species' vertical LiDAR return distribution, and could create counter-intuitive effects. 
Mature spruces for example, could have an increased probability of top reflection because they often display large crown tops filled with cones, which  raises the highest stand return and in turn creates an accumulation of points in the lower part of the stand. 
Furthermore, in young stands where a single mature tree might remain, the vertical distribution of LiDAR returns could be compressed. 
Using a height quantile, such as 95% height, rather than the highest return could reduce this effect and improve model performance.
In addition to compression, the vertical structure can be distorted by topography and returns from the inferior parts of the stands are  subject to occlusion; these effects could be mitigated [@roussel_etal17; @roussel_etal18; @wilkes_etal16; @liu_etal17].
...
under the presence of confounding variables [@winkler_etal14; @anderson_robinson01]. 
It also has the advantage of the graphical interpretation, especially the identification of the areas of rejection, which allowed us to interpret at which relative heights the differences in species, crown cover, and age occured.
We think that this method could complement and sometimes replace other methods of analysis of the vertical distribution of LiDAR returns, such as principal component analysis or linear discriminant analysis [@fedrigo_etal19], when interpretability is desired.
...
In this paper, we applied the method to a selection of simple stands (clear species dominance, and even-aged) to visually understand the effect of each of these factors.
