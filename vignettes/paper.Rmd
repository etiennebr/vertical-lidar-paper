...
after accounting for stand crown closure and age
...
economical data (Mrkvička et al. 2020), and similar non-parametric GLMs are
...
discrete factor and the coefficients are constrained to sum to zero.
...
where species in columns are subtracted from the species in rows (e.g. first 
...
curve is bolder when outside the envelope. 

Panels above the diagonal are the reflection of the observed functions 
...
20%). Although black spruce peak was centered at 50% stand height for high values of crown closure, the observed
...
```{r include = FALSE}
fig_cc_caption <- 
"Nonparametric graphical tests of significance for crown closure:
the observed coefficient (black curve), 
and the 98% global envelope (grey band) that shows the area of acceptance of 
the null hypothesis (no effect, <0.001) obtained from the permutations of the 
residuals of the null model (Equation \\@ref(eq:cc)). The observed curve is 
bolder when outside the envelope."
fig_cc_short_caption <- "Nonparametric graphical tests of significance for crown closure"
```
...
advantage of the graphical interpretation, especially the identification of the
 area of rejection, which allowed us to interpreted at which relative heights 
