...
We assessed even-aged stand vertical distributions of LiDAR returns and found that species, age, and crown cover each have a distinct pattern that together explain up to 47% of the variation.
...
One limitation of LiDAR in this context is the high dimension of the data requiring dimension-reduction techniques that limit the interpretation of results.
We quantified the influence of three variables, species, crown cover, and age, on the vertical distribution of airborne LiDAR returns from forest stands. 
...
We modeled the vertical distribution of LiDAR 
returns against the three variables using a functional general linear model and a novel nonparametric graphical test of significance.
...
economics data (Mrkvička et al. 2020), and non-parametric inference is
...
(41–60), 70 (61–80), 90 (81–100), and 120 (101, ∞), and one irregular age class (such as uneven-aged and multi-stratum stands).
...
Using a low threshold for dominance increased the number of usable observations for the analysis. 
...
number of LiDAR returns. From the remaining stands, we kept only those with a dominant species that was observed at least in a 100 stands: aspen,
...
\begin{equation}
d(h) = \beta_0(h) +X_{sp}\cdot\beta_{sp}(h) +X_{cc}\cdot\beta_{cc}(h) +X_{age}\cdot\beta_{age}(h) +\varepsilon(h)  (\#eq:full)
\end{equation}
<!-- d(h) is a N \times 1 vector containing the observed density distributions for all $i$ -->

where, for every scaled height $h$, $d(h)$ is the $N\times1$ vector of observed LiDAR density distributions at scaled height $h$, and
...
Following @mrkvicka_etal20, we transformed the matrix of vertical LiDAR returns density (Figure \@ref(fig:fig-fglm-process)) by scaling groups ($j$)
according to their variance dispersion. The initial $d_{i,j}$ function was
transformed into a $S_{i,j}$ (scaled) function for all $i = 1, ..., N$,
...
confirm that we can reject the null hypothesis, as the differences between
...
However, we expected clearer vertical distribution patterns along shade-tolerance gradients or between conifer and deciduous trees; we found that patterns of paper birch were more similar to those of balsam fir (a shade-tolerant confirm) than aspen, another shade-intolerant deciduous.
...
While fGLM is not suited for making predictions, methods such as Deep Neural Network, could be good candidates for species classification using LiDAR distributions because they can handle the functional representation of the data and their non-linear relations. 
...
<!-- age -->
Increase age was associated with the vertical distribution of LiDAR returns being displace higher in the stand for up to 70 years, followed by a plateau or decline in the 90- and 120-year-old groups. 
...
Crown cover, however, seemed to explain the variation in the vertical distribution of LiDAR returns below 21% of stand height and 46--67% stand height, while age explained the variation between 18--38% and 62--64% of stand height. 
Unlike @aber79 who observed 
a stable distribution of foliage at the end point of forest succession, the
vertical distribution of LiDAR returns of the 120 year group  
displayed a small decreased concentration of returns below 33--38%.
...
In addition to compression, the vertical distribution can be distorted by topography and occlusion of lower stand sections. 
These effects are increasingly documented and can be mitigated by accounting for LiDAR survey parameters and topography [@roussel_etal17; @roussel_etal18; @wilkes_etal16; @liu_etal17]. 
...
The intersection of these two distributions might create noise which could then be amplified by curve registration.


<!-- fglm -->
The nonparametric graphical test of significance that we used is slightly
liberal because is is based on the Freedman-Lane algorithm which uses an approximation from permutations [@mrkvicka_etal19]. 
