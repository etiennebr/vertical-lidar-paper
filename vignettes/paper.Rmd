...
# Make sure this is run: here::here("vignettes/contrasts.Rmd")
```
...
techniques to a large area is very difficult using traditional
...
known to record a 3D scene. A Discrete LiDAR pulse is reflected by the
vegetation as it penetrates the canopy. The sensor then records the energy
returned by the pulse and the total travel time to deduce the distance from the
sensor to the point of reflection. LiDAR can be used from the ground, and from
...
Some studies explicitly compared vertical the distribution of LiDAR returns between
species. For example, @orka_etal09 found that first and last returns were more
dispersed in Birch (*Betula* spp.) than in Norway spruce (*Picea abies* (L.)
Karst.), which also had a vertical distribution of first returns skewed toward
the superior canopy, and that increased stand height modified the overall return
distribution.
...
```
Footnote Table \@ref(tab:species) :

1: Colors are based on (NIR+G+B mapped to R, G, B channels).

2: Adapted from @farrar95a and @leboeuf_vaillancourt13 .
...
Even-aged stands are less complex to analyze from a vertical structure point
of view than irregular stands, so we focused our analysis on even-aged stands
with clearly dominant species representing at least 50% of the stand cover. We
excluded stands with an average sampling rate below 2 pt/m², or an area of
less than 4 ha to ensure a sufficient number of LiDAR returns. From these stands,
we kept only those with a dominant species that had at least 100 observations
(stands): aspen, balsam fir, black spruce, paper birch, and white spruce. From
22 365 segmented stands of the original forest map, we reduced our dataset to
5 428 stands representing 35% of the study area (Figure \@ref(fig:fig-loc)).
Stand area distribution was similar for every selected species (median 9 ha,
minimum 4 ha, maximum 137 ha).
...
envelope test* to build a null global envelope in which our null hypothesis (no
differences between distributions) is accepted [@myllymaki_etal17]. The null
...
heteroscedasticity using the squared residuals $(d_i(h) - \hat{d_i}(h)) ^ 2$ on
...
above 38% increased (Figure \@ref(fig:fig-cc)). The largest augmentation effect was
concentrated around the middle height (50%), while the inferior area (around 3%
of height) had the largest reduction effect. Figure \@ref(fig:fig-cc2) displays the effect of crown
...
for stands between 10 and 70 years of age ($p$<0.001), while significant effect for stands
...
years, where there is a positive effect of age between 38–97% of stand height
(44–100% for 70 yr), and a negative effect below 31% of stand height (38% for
...
groups were located around 5, 20, and 50% of stand height. Figure \@ref(fig:fig-age2)
...
Increased crown closure was associated with a concentration of the
vertical distribution of LiDAR returns which moved points below 33% to
above 38% of stand height. Complementarily, stands with a lower crown closure
presented a vertical distribution of LiDAR returns that was more
dispersed and skewed toward the ground, with fewer returns in the
...
hypothesis that the distributions of LiDAR returns were similar, and also
...
The exact optimal area to observe these patterns will require
additional research.
...
discriminating species, crown closure, and age based on the vertical
