...
and ranging (LiDAR) can measure the position of
...
novel nonparametric graphical test of significance to identify the effect of 
the variables on the distributions. 
...
concentrated the vertical distribution of returns c. 50% height. Age
gradually shifted the distribution higher in the stand until 50–70
years, plateaued and slowly declined at 90–120 years. The full model maximal R²
was 0.47 c. 10% of stand height. Results suggest that the stands
...
Traditionally, characterization of stand vertical distributoin of aboveground 
...
increasingly becomes a collection of random deviates. For that reason, accuracy
in species identification is improved by increasing point density up to 10 pt/m²
[@fassnacht_etal16].

Aerial LiDAR is also used in area-based approaches (see e.g. @white_etal13 for
an exhaustive description). This approach generally uses a stand or a pixel on
...
In a study of 14 areas in New Hampshire, @aber79 observed that the vertical
concentration of the foliage evolved across stand age and the end point of the 
forest succession seemed to reach an equilibrium where the foliage was relatively 
evenly distributed within the canopy.

Some studies explicitly compared the vertical distribution of LiDAR returns
between species. For example, @orka_etal09 found that first and last returns
were more dispersed in Birch (*Betula* spp.) than in Norway spruce (*Picea
abies* (L.) Karst.), which also had a vertical distribution of first returns
skewed toward the superior canopy, and that increased stand height modified the
overall return distribution. These observations can be useful to characterize 
the habitat and understand the forest structure dynamics.
...
interspecific variations after accounting for stand crown closure and age.
...
# }
knitr::include_graphics("pictures/fig-location.png")
```
...
concentrated around the middle height (c. 50%), while the inferior area (around
...
superior (c. 80% of stand height) and white spruce which peak was inferior (c.
20%). Although black spruce peak was centered c. 50% stand height, the observed
...
```{r include = FALSE}
fig_rsq_caption <- 
"$R²(h)$ of each model. Each label indicates the minimal model, which exludes the 
...
fig_rsq_short_caption <- "$R²(h)$ of each model"
```


```{r fig-rsq, fig.cap=fig_rsq_caption, fig.scap=fig_rsq_short_caption, echo = FALSE}
knitr::include_graphics("figure/fig-r-squared.svg")
```
...
distributions skewed below c. 30% of stand height. Increase in crown 
