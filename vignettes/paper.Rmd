...
strategies vary by species resulting in archetypal tree morphologies that will
have predictable, although complex, reactions to growth conditions. This results
in the distinct species-specific tree shapes.
...
function), which are used to predict stand attributes [@magnussen_etal99;
...
The ability to use low-density LiDAR surveys for species classification would
provide an extensive species mapping answering the need of forest management
because the very-high point density LiDAR surveys needed by the tree crown
approaches are generally not available. For that reason, we investigate the link
between species and the vertical distribution of LiDAR returns from
lower-density surveys by using a novel method called the graphical test of
significance [@myllymaki_mrkvicka19; @mrkvicka_etal19]. The test allows for a
visual understanding of functional data---such as a the vertical distribution of
LiDAR returns. It provides a framework to perform non-parametric tests to
compare a continuous function (i.e. the vertical distribution of LiDAR returns)
against categorical and continuous variables.
 
## Forest vertical structure

The information contained in the vertical distribution of LiDAR returns is
closely related to the distribution of the vertical distribution of the
vegetation, which has been linked to leaf-area index, and other forest
attributes. However, when provided, the interpretation of the distribution of
the LiDAR return is often limited and hardly generalizes to other studies
[fassnacht_etal16]. For that reason, methods that have the potential to
generalize and eventually allow to build a theoretical framework are much
needed.

Some studies explicitly compared the vertical distribution of LiDAR returns
between species. For example, we noted that @orka_etal09 found that first and
last returns were more dispersed in Birch (*Betula* spp.) than in Norway spruce
...
@aber79 studied the evolution of foliage distribution in a study of 14 areas in New
Hampshire. He observed that the vertical concentration of the foliage evolved
across stand age, and the end point of the forest succession seemed to reach an
equilibrium where the foliage was relatively evenly distributed within the
canopy. While these observations of foliage distribution do not apply directly
to the vertical distribution of LiDAR returns, they are likely to influence the
LiDAR returns.

## Objectives

In this paper, we combined principles from the tree-crown approach to species
identification, the vertical biomass distribution, and the area-based method to
compare the vertical distribution of LiDAR returns of different species, crown
closure, and ages, which are known to affect the stand structure. We speculate
that intraspecific variations of LiDAR distribution patterns are smaller than
interspecific variations after accounting for stand crown closure and age. Our
approach relies on the introduction of the non-parametric graphical test of significance, 
which can provide an intuitive understanding of the vertical distribution of 
LiDAR returns.
...
remaining 72% of the territory had an undocumented origin. 
...
distributios were similar for all selected species (median 9 ha,
...
statistical test. For the continuous crown closure variable, the test statistic
...
coefficients $\beta_{sp}m(h)$ of categorical variables were constrained to sum to
zero.
...
heterogeneity of variance [@winkler_etal14], and the homoscedasticity tests of
@mrkvicka_etal20 indicated heterogeneity of variance for all variables.
...
important because it can reintroduce significant heterogeneity of variance. We
settled on the successive transformation by crown closure, age, and species
which provided the best results and reduced heterogeneity, but did not
completely remove it according to the test from @mrkvicka_etal20. Our
simulations showed that heterogeneity could remain significant after successive
transformations, especially for a large dataset of 5000 observations, which is a
common problem of null hypothesis testing on large datasets. We thus looked at
...
distribution of LiDAR returns. The increase in crown closure concentrated the
...
variability was skewed toward the inferior stand height.
...
shaded areas represent 95% and 50% variation envelope."
...
a strong assymetry toward the inferior part of the stand that disappeared in
older stands.
...
```{r include = FALSE}
fig_rsq_caption <- 
"Comparison of $R²(h)$. The minimal model excludes the 
variable of interest. For examples, the Species model explicitly excludes the 
$Species$ variable (Eq. \\@ref(eq:species)). The full model 
(Eq. \\@ref(eq:full)) contains all variables."
...
stands of a species exhibited similar vertical characteristics that distinguished
them from other species. This observation consistent with previous field 
...
are relative to the maximal height of the stand. 
...
The area 31--46% of stand height exhibited a shard drop in R², where age seemed
...
The vertical distribution of LiDAR returns for each stand was dependent on the
...
quantile rather than rather than the highest return could reduce this effect.
...
LiDAR parameters and abiotic factors, could be included to improve the model.
...
distribution below 33%, while age affected a larger part 
