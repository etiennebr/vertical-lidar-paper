...
nocite: |
  @farrar95a, @leboeuf_vaillancourt13, @leboeuf_vaillancourt13a
---
...
One limitation of the airborne LiDAR data is its inability to distinguish differences in foliage condition or in species spectral variation. 
This is because it lacks the spectral information commonly used to classify species such as
variations in red, green, and blue or near-infrared multispectral imagery. 
In response, the most common strategy for distinguishing species using LiDAR has been to differentiate individual tree shapes and texture [@holmgren_persson04; @kim_etal11; @fassnacht_etal16], and add spectral information from multispectral imagery. 
More recently multispectral LiDAR has also been used to distinguish stand species [@budei_etal18; @budei_st-onge18].
...
This is due to a decrease in the number of LiDAR returns, causing the distribution to become a collection of random variates. 
...
Predictors are often derived from the vertical distribution of LiDAR returns: the number of LiDAR returns per height slice. 
...
x <- tibble::tribble(
    ~Species, ~Silhouette, ~"Description\\textsuperscript{1, 2}", ~"Associated species\\textsuperscript{2}",
...
tbl_species %>% 
 #dplyr::mutate(Silhouette = "Silhouette") %>% 
  ...
  kableExtra::footnote(number = c("Colors are based on (NIR+G+B mapped to R, G, B channels).", "Adapted from Farrar (1995); Leboeuf and Vaillancourt (2013a, 2013b)"))
```
...
However, we found that using a 60% dominance made little difference to the conclusions, whereas a higher threshold removed too much data for some species.

The results in this paper demonstrate that vertical LiDAR return distribution can be useful for species classification.
Based on these findings, models could be trained to link these vertical distributions to species and then used to estimate species occurrence over the landscape.
Many studies performing species classification with LiDAR use the vertical distribution of LiDAR returns [@fassnacht_etal16], however the approach developed here allows the drivers of cover and age to also be integrated into the estimation, thereby making the species prediction more robust when applied more broadly.
...
Furthermore, the occlusion of vegetation in lower stand parts underestimates the density of vegetation. 
Some of the noise in the vertical distribution can be mitigated by using correction methods such as voxels, or partly accounting for laser incidence angle, footprint size and pulse density [@roussel_etal17; @roussel_etal18; @wilkes_etal16]. 
