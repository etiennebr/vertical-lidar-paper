...
cset_trans_age <- corrUnequalVar(cset_trans, groups=with(independent_factors, age_class))
# transform only if heteroscedastic
pd <- my_plot_data_combined_global_envelope(res_fanova_sp)
height_inside <- pd$ribbon %>% 
  filter(
    type == "Data function"
  ) %>% 
  group_by(r) %>% 
  summarise(inside = all(inside), .groups = "drop_last") 

cset_trans$funcs[!height_inside$inside] <- cset_trans_age$funcs[!height_inside$inside,]
