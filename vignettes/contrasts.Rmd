...
nperm <- 3000  # number of permutations
...
ggsave("figure/fig2-sp-combined.pdf", pcsp, device = device, dpi = "retina", width = 234, height = 234, units = "mm")
```
...
ggsave("figure/fig3-glm-cc.pdf", pc_cc, device = device, dpi = "retina", width = 234 / 2, height = 234 / 2, units = "mm")
```
...
ggsave("figure/fig4-cc.pdf", pcc, device = device, dpi = "retina", width = 234, height = 234, units = "mm")
```
...
ggsave("figure/fig5-cc.pdf", page, device = device, dpi = "retina", width = 234, height = 234 / 2, units = "mm")
```
...
papu <- wave_long %>%
  filter(!trans) %>%  # use untransformed data
  ...
  geom_line(data = wave_long, aes(y = ss, color = dominant_species, group = stand_id), show.legend = FALSE, size = 0.1, alpha = 0.4) +
  ...
  geom_text(data = wave %>% count(dominant_species, crown_closure, age_class), aes(label = n), x = Inf, y = Inf, hjust = 1, vjust = 1, size = 3) +
  facet_grid(dominant_species + age_class ~crown_closure) +
    labs(
    title = "Vertical distribution of lidar returns (untransformed)",
    subtitle = "Figure A-1", 
    caption = wrapper(cap, 80)
    )

papt <- wave_long %>%
  filter(trans) %>%  # use transformed data
  group_by(dominant_species, crown_closure, age_class, height) %>% 
  summarise(
    n = n(),
    median = median(ss),
...
ggsave("figure/appendix.pdf", pap , width = 6, height = 22, units = "in")
```
