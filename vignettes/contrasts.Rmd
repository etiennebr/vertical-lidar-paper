...
functional_anova_homogeneity <- function(g, nsim = nperm, contrasts = FALSE, curve_set = cset_trans, ...) {
  GET::graph.fanova(
    nsim = nsim,
    curve_set = curve_set,
    groups = g,
...
First, we need to test for variance homogeneity. If there is heteroscedasticity, 
then we apply a transformation, and check that heteroscedasticity decreased.
```{r}
...
res_fanova_sp <- functional_anova_homogeneity(independent_factors$dominant_species, curve_set = cset)
plot(res_fanova_sp) + coord_flip()

# Successively equalize variance for each group, then test
cset_trans <- corrUnequalVar(cset, groups=with(independent_factors, crown_closure))
res_fanova_sp <- functional_anova_homogeneity(independent_factors$dominant_species, contrasts = FALSE, curve_set = cset_trans)
...
# This figure shows the successive effect of the transformations
pa + pb + pc
```
...
```{r}
res_fanova_sp <- functional_anova_homogeneity(independent_factors$dominant_species, contrasts = FALSE, curve_set = cset)
res_fanova_sp_trans <- functional_anova_homogeneity(independent_factors$dominant_species, contrasts = FALSE, curve_set = cset_trans)
res_fanova_cc <- functional_anova_homogeneity(factor(independent_factors$crown_closure), contrasts = FALSE, curve_set = cset)
res_fanova_cc_trans <- functional_anova_homogeneity(factor(independent_factors$crown_closure), contrasts = FALSE, curve_set = cset_trans)
res_fanova_age <- functional_anova_homogeneity(independent_factors$age_class, contrasts = FALSE, curve_set = cset)
res_fanova_age_trans <- functional_anova_homogeneity(factor(independent_factors$age_class), contrasts = FALSE, curve_set = cset_trans)
...
plot(res_fanova_age) + coord_flip() + plot(res_fanova_age_trans) + coord_flip()
```

## GLM

The envelopes are created from `r nperm` permutations. We'll use weighted least 
squares to remove any potential remaining heteroscedasticity. We use inverse 
variance weights (`1/var(Y)`).
```{r}
wave_long <- wave %>%
  pivot_longer(
    cols = matches("ss_"),
...
weights <- by_height %>% 
  mutate(
    data = map(data, ~group_by(.x, dominant_species, crown_closure, age_class) %>% mutate(weight = 1/var(ss)))
  ) %>% 
  unnest(data) %>% 
  group_by(stand_id) %>% 
...
res_flm_sp <- functional_lm(Y ~ age_class + crown_closure, lm.args = list(weights = weights$weight))
res_flm_cc <- functional_lm(Y ~ dominant_species + age_class, contrasts = FALSE, lm.args = list(weights = weights$weight))
res_flm_age <- functional_lm(Y ~ dominant_species + crown_closure, contrasts = FALSE, lm.args = list(weights = weights$weight))
```
