...
ggsave("res_trans.pdf", plot = q, width = 12*2, height = 8*2, units = "in")

```

Since there are a lot of zeros in the upper portions, the log transform doesn't seem to work, and the boxcox transformation either. 
It might be that the variance is unequal between groups, which is something we observed. For that reason, we could try to weight each model based on the inverse of group variance. 
```{r}
sp_mod <- function(x) {
