...
```{r setup}
devtools::load_all("~/workspace/vertical-lidar-paper")
library(GET)
library(patchwork)
library(tibble)
library(ggplot2)
...
The envelopes are created from `r nsim` permutations.

```{r}
functional_lm <- function(null_model, nsim = nperm) {
  graph.flm(
    nsim = nsim, 
...
res_flm_cc <- functional_lm(Y ~ dominant_species + age_class)
res_flm_age <- functional_lm(Y ~ dominant_species + crown_closure)
```

### Contrasts
```{r}
p <- my_plot_contrasts(res_flm_sp, 39, full = TRUE)
u <- my_plot_contrasts(res_flm_cc, 39, full = TRUE)
v <- my_plot_contrasts(res_flm_age, 39, full = TRUE, diag = TRUE) +
  scale_y_continuous(breaks = c(-0.2, 0, 0.2))
# we need to remove the diagonal manually after overriding the y scale
v <- remove_diag_panels(v)
