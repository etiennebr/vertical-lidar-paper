...
corrUnequalVar <- function(curve_set, groups, ...) {
  if (!is.factor(groups)) {
...
ggplot(data = by_height) + 
  geom_path(aes(sp_wls_mvr_res, height), color = "blue") + 
  geom_path(aes(sp_mvr_res, height)) + 
  geom_path(aes(sp_mvr, height), color = "red") + 
  geom_vline(xintercept = 4) +
# kurtosis should 0
#qplot(sp_kurtosis, height, data = by_height) + geom_path() + geom_vline(xintercept = 0) 
ggplot(data = by_height) + 
  geom_path(aes(sp_wls_bptest_statistic, height), color = "blue") +
  geom_path(aes(sp_bptest_statistic, height))
qplot(sp_wls_bptest_p_value, height, data = by_height) + geom_path()
```
