...
res_var <-list(
  sp = functional_anova(independent_factors$sp, contrasts = FALSE, curve_set = cset),
  age = functional_anova(independent_factors$age, contrasts = FALSE, curve_set = cset)
)

(my_plot_contrasts(res_var$sp, 10) + labs(title = "sp")) + 
  (my_plot_contrasts(res_var$age, 10) + labs(title = "age"))
...
```{r}
cset_trans_3 <- corrUnequalVar(cset_trans, groups=with(independent_factors, interaction(sp, age)))
res_var_trans_3 <-list(
  sp = functional_anova(independent_factors$sp, contrasts = FALSE, curve_set = cset_trans_3),
  age = functional_anova(independent_factors$age, contrasts = FALSE, curve_set = cset_trans_3)
)

(my_plot_contrasts(res_var_trans_3$sp, 10) + labs(title = "sp")) + 
  (my_plot_contrasts(res_var_trans_3$age, 10) + labs(title = "age"))
```
